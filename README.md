# Резюме на вакансию  Fullstack QA Engineer / AQA Engineer Python
<img src="https://user-images.githubusercontent.com/122479950/221151694-71c9199b-257b-4a7e-9efd-3acea36638c7.JPG" width="200" />

**Павел Михайлович Мысливец**

## Специализации:
__Fullstack QA Engineer / AQA Engineer Python__
- _Опыт работы: 4 года 
*** 
### Контакты для связи:
 - 7 (952) 390-99-51 (звонок / whatsapp)  — предпочитаемый способ связи
 - https://t.me/pavlik_masl
 - Email: myslivets0910@yandex.ru
 - LinkedIn: Pavel Myslivets www.linkedin.com/in/pavlik-masl
 - Вконтакте / Facebook / Instagram @pavlik_masl

### Иностранные языки:
1. Русский - родной
2. Белорусский - родной
3. Английский  - на уровне чтения документации

## Hard Skills:
- Автоматизация тестирования Web и API сервисов на Python (Pytest, Playwright)
- Разработка, поддержка и выполнение автоматизированных автотестов для web-приложений и REST API на Python (Pytest, Playwright)
- Поддержка и развитие тестовой инфраструктуры, внедрение новых инструментов в процессы контроля качества
- Проектирование тест-кейсов, создание чек-листов и баг-репортов, подготовка тестовой документации (TestRail, Jira, Qase.io)
- Unit-, интеграционное, регрессионное, приёмочное тестирование
- CI/CD: Jenkins, GitHub Actions
- Тест-дизайн, покрытие тест-кейсами с привязкой к требованиям
- Работа с REST API: Postman, Swagger
- Работа с SQL (DBeaver, PhpMyAdmin), создание запросов, валидация данных
- DevTools (Chrome, Firefox, Opera), анализ и документирование логов (DevTools, Sentry, Kibana)
- Оформление баг-репортов, ведение тестовой документации (Jira, Qase.io, Confluence)
- Performance testing: JMeter
- Опыт работы с микросервисами и клиент-серверной архитектурой
- Аналитика: Allure, Amplitude, Яндекс.Метрика

- Анализ логов и взаимодействие с DevOps для устранения дефектов в тестовой среде и CI/CD пайплайнах
- Функциональное, нефункциональное, кроссбраузерное тестирование (DevTools, Mobile FIRST, SHAREit)
- Передача знаний команде и проведение внутренних обучающих сессий для новых специалистов



__тест-кейсы [ПРИМЕРЫ](https://github.com/myslivets0910/Career-Resume.md/blob/5791bb4cb5e80712b65d2c7c5e75ba6cdd3a8c22/test-case.md) / чек-листы / баг-репорты [ПРИМЕРЫ](https://github.com/myslivets0910/Career-Resume.md/blob/fad11cbfb249efd2bb4cd7f2b44e1caa110a3bb7/bug-reports.md) / отчет о тестировании;__
- Документирование найденных дефектов и оформление баг-репортов;
- **Чтение и документирование логов;**
- **Написание автотестов на Pytest + Allure** [Pet-проект Pytest + Allure](https://github.com/myslivets0910/auto_ART.git)
- **Знаком с методологиям Agile, Scrum**

## СТЕК:
- Языки и FW: Python, Pytest, Playwright, bugbug.io, SQL
- CI/CD: Jenkins, GitHub Actions, Docker, Git
- Документация и баг-трекинг: Jira, Qase.io, Confluence, Xmind, Miro
- API: Postman, Swagger.io, SoapUI
- DB: PostgreSQL, DBeaver, PhpMyAdmin
- Аналитика и отчетность: Allure, Amplitude, Яндекс.Метрика
- Мониторинг: Sentry, Kibana
- UI/UX testing: PerfectPixel, Google Lighthouse, CSSViewer

## MOBILE APP TESTING 
**Тools:**
 - DevTools / Mobile FIRST / SHAREit

## Soft Skills:
- Развитые навыки коммуникации и взаимодействия с командами разработки и бизнеса
- Умение быстро обучаться и осваивать новые технологии/инструменты
- Самостоятельность, проактивность и ответственность за результат
- Внимательность к деталям, системное мышление, умение приоритизировать задачи
- Высокая стрессоустойчивость, способность работать в условиях быстро меняющихся требований


## [ОПЫТ РАБОТЫ](https://github.com/myslivets0910/Career-Resume.md/blob/6ad14d6922c77f0e87d7d26d8b7a2ef58411e27c/work-experience.md)
## [TEST-CASE](https://github.com/myslivets0910/Career-Resume.md/blob/5791bb4cb5e80712b65d2c7c5e75ba6cdd3a8c22/test-case.md)
## [BUG-REPORTS](https://github.com/myslivets0910/Career-Resume.md/blob/fad11cbfb249efd2bb4cd7f2b44e1caa110a3bb7/bug-reports.md)
## [Sales experience](https://github.com/myslivets0910/Career-Resume.md/blob/main/Sales-experience.md)
## [Marketing experience](https://github.com/myslivets0910/Career-Resume.md/blob/9fa3cb51c6304c8d1a501df7bc7e86e1d2140f43/Marketing-experience.md)
## [Pet-проект Pytest + Allure](https://github.com/myslivets0910/auto_ART.git)


## Образование:
___Российская таможенная академия (филиал Санкт-Петербург)___
- _Факультет: таможенное дело_
- _Специалист таможенного дела_
- _Год окончание 2015_

## [О СЕБЕ](https://github.com/myslivets0910/Career-Resume.md/blob/e3779f01eacc97fafd0b674702bfe74000ff3086/about-me.md)

## Пройденные программы:

### СКБ Контур 2023 "ULearn.me - **Тестирование ПО**
-

### ООО «Нетология» 2022 "Тестировщик ПО" - **Ручное тестирование веб-приложений**
-
<img src="https://github.com/myslivets0910/portfolio/assets/122479950/5c296c26-b93e-4d41-9207-8ff328ebabde" width="400" />

### «Кавычки» 2023 "Вселенная тестирования, или Как стать тестировщиком" - **Тестирование ПО**"
-
<img src="https://github.com/myslivets0910/portfolio/assets/122479950/d8855325-7cef-46d2-8eb2-9aa0b9a940aa" width="400" />

### QA Way 2023 "Тестирование ПО: Postman для тестирования API" - **Тестирование API**
-
<img src="https://github.com/myslivets0910/portfolio/assets/122479950/5eb247f3-8c86-486c-ab0e-1859272506ce" width="400" />

### ООО «Нетология» 2022  - **Git - система контроля версий**
-
<img src="https://github.com/myslivets0910/portfolio/assets/122479950/cc71f2d1-e20d-453e-859d-2c266802840d" width="380" />

### ООО «WL Company» 2023  - **"WEB-разработчик: Основы"**
-
<img src="https://github.com/myslivets0910/Career-Resume.md/assets/122479950/b1f526d9-527f-4edb-9f4f-346de5df18d4" width="380" />

-
### 2024 **Тестирование REST API в Postman**
<img src="https://github.com/user-attachments/assets/8d4e9dfd-8563-4a1b-8db2-d90bdcb1099d" width="380" />

### 2025 **Автоматизация тестирования с помощью Selenium и Python**
<img width="380" alt="S_auto" src="https://github.com/user-attachments/assets/9a75073e-3e71-4fbf-9f4a-089ad029f263" />


